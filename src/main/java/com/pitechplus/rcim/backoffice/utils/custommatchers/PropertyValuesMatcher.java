...
public class PropertyValuesMatcher<T> extends TypeSafeDiagnosingMatcher<T> {
    ...
    private static <T> List<PropertyValuesMatcher.PropertyMatcher> propertyMatchersFor(T bean, PropertyDescriptor[] descriptors) {
    ...
    public static <T> Matcher<T> samePropertyValuesAs(T expectedBean) {
        return new PropertyValuesMatcher<T>(expectedBean);
    }
